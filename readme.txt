Collage is a matlab programm that combines a plethora of given pictures to a collage that was specified by the user
...
pictures this function loops through every individual pixel and views the RGB value. Than the average of each 
...
enter the name of the directory where his/her pictures are saved. This directory has to be located within the 
...
wether a picture has been taken horizontally and rotate that in order to correct the format. The new size will
...
original .png files from being overwritten. Than, each pixel will be manually darkend if possible. Currently, 20 
is substracting from each component of each RBG value, seems to work fine but can also be more or less.
